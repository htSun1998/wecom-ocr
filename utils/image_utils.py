...
def is_image(image: np.ndarray, point: tuple=CHAT_POINT, target_color: tuple=CHAT_COLOR) -> bool:
    # 检查高度
...
def compare_images(image1: np.ndarray, image2: np.ndarray) -> int:
...
def load_cv2(img: np.ndarray, grayscale=True) -> np.ndarray:
